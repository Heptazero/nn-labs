...
sparsemax 的坍缩轨迹是否不同，以及这些轨迹是否与正确、错误和稳定混合
等结局相关。
...
- 开发 seed 的首次扫描已经触发上述条件：`rho=0.40` 时 softmax 在
  `alpha=12` 的 median IPR 仍为 `38.92`，而 sparsemax 在 `alpha=0.25`
  的最高 median IPR 只有 `34.91`。因此在任何正式 seed 运行前，一次性
  冻结扩展为 `0.05:0.05:0.50`、`0.60:0.10:2.00`、
  `2.25:0.25:4.00`、`4.50:0.50:12.00`、`13:1:24`，再加
  `{0.75,26,28,30,32,36,40,48}`。扩展只用于第一步 IPR；终态类别覆盖仍只在
  原 11 点网格计算，避免把 100 步终态扫描复制到 69 个校准点；
...
选择集上两种方法各自相对冻结水平的误差必须不超过 5%；留出集上两种
方法的 median IPR 相对差必须不超过 5%。否则条件标为 `unmatched`，不
外推。留出集相对冻结水平的漂移另行报告，但不与方法间匹配误差混为一项。
