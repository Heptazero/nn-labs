...
先看 [问题目录与实施计划](./QUESTIONS.md)：列出 13 个问题、已有入口、缺口、模型位置、分文件方案和 Colab 起步预算。下一项优先完善“继续检索是在纠错，还是强化错误”。

当前状态（2026-09-05）：[第一阶段 Notebook](./hopfield_benchmark_phase1_colab.ipynb) 已内联实现六个模型适配器、配对运行器和图表入口，仓库版本未保存运行输出；尚未拆出共享模型文件，不能据此声称跨模型实验已验证。完整比较约定见 [PROTOCOL.md](./PROTOCOL.md)。
