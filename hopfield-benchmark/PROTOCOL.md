...
状态：`v0.1 / protocol-only`

本版本只固定问题、接口、指标、预算与停止条件。它不包含模型实现，也不产生实验结论。
...
当前已完成 Gate 0 协议和 Phase 1 adapter 骨架，但尚未在 Colab 执行 Gate 1。下一步是先验收两条候选基线；在基线可信度通过前，不把同图曲线写成模型优劣结论。
